...
\newlength{\methodfigwidth}
\setlength{\methodfigwidth}{6.5in}
...
\begin{tikzpicture}[
  ...
  \node[box=gray, text width=0.26\methodfigwidth] (bc) {
  ...
  \node[box=teal, text width=0.29\methodfigwidth, right=0.55cm of bc] (pair) {
    \textbf{2. Counterfactual instruction pair}\\[2pt]
    Keep the same observation and action:
    $(o_t,a_t)$\\[3pt]
    \begin{tikzpicture}[font=\scriptsize]
      \node[tealbox, text width=0.255\methodfigwidth] (pos) {correct instruction $\ell_i$\\score $s^+=\log p_\theta(a_t\mid o_t,\ell_i)$};
      \node[orangebox, text width=0.255\methodfigwidth, below=3pt of pos] (neg) {wrong-target instruction $\ell_i^-$\\score $s^-=\log p_\theta(a_t\mid o_t,\ell_i^-)$};
    \end{tikzpicture}\\[-2pt]
    The negative changes the task target, not the image or expert action.
  };

  \node[box=orange, text width=0.30\methodfigwidth, right=0.55cm of pair] (loss) {
