...
ALPHA=${ALPHA:-0.2}
TAG=${TAG:-a020}
RUN_ID=interp_rbtad_anchor_${TAG}_seed7_b20
RUN_ROOT=${ROOT}/runs/interp_rbtad_anchor/${RUN_ID}
...
{
  echo "== Interp RBTAD/Anchor start $(date -Is) alpha=${ALPHA} =="
  ...
  save_root="results/interp_rbtad_anchor/${RUN_ID}/0/interp_${TAG}_30trials_egl_${stamp}"
  ...
  python vla_scripts/parallel_libero_evaluator_egl.py \
    ...
    --pretrained-checkpoint "runs/interp_rbtad_anchor/${RUN_ID}" \
    --unnorm_key libero_core_lt \
    --save-root "${save_root}" \
    --steps 0 \
    --instruction-formatting False

  echo "== Interp RBTAD/Anchor eval done $(date -Is) =="
