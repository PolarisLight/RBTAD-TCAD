...
run_one() {
  local mode="$1"
  local port="$2"
  local run_root="runs/cross_dataset_probe"
  local run_id="${mode}_${SUITE}_s${LIMIT_STEPS}_seed7_b20"
  ...
  if [[ "${mode}" == "rbtad" ]]; then
    tcad_lambda="0.1"
    tcad_ratio="0.5"
    # Standard LIBERO suites are not long-tail splits; include all tasks for
    # counterfactual discrimination, and keep rare weighting neutral.
    tail_max_count="1000000000"
  ...
  echo "== ${mode} train done $(date -Is) status=${train_status} =="
  ...
  local save_root="results/cross_dataset_probe/${run_id}/${eval_step}/${mode}_${SUITE}_${NUM_TRIALS}trials_egl_${stamp}"
...

{
  ...
  mkdir -p "$PRISMATIC_DATA_ROOT" "$HF_HOME" runs/cross_dataset_probe results/cross_dataset_probe
  : > .hf_token

  nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits || true
  run_one baseline 29601
  run_one rbtad 29602
