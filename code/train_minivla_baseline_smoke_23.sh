...
LOG=/mnt/data/cyh/vla_train_smoke_23.log

{
  ...
  echo "== command =="
  echo "torchrun --nnodes 1 --nproc-per-node 1 --master_addr 127.0.0.1 --master_port 29542 vla_scripts/train.py ..."

  torchrun --nnodes 1 --nproc-per-node 1 --master_addr 127.0.0.1 --master_port 29542 vla_scripts/train.py \
    ...
    --run_root_dir runs/smoke_baseline_full_23 \
    --run_id smoke_full_1gpu_1step \
    --max_steps 1 \
    --save_interval 999999

  echo "== smoke done $(date -Is) =="
  find runs/smoke_baseline_full_23/smoke_full_1gpu_1step -maxdepth 3 -type f -printf "%p %s\n" | sort
} >> "$LOG" 2>&1
