...
LOG=/mnt/data/cyh/rabad_full_23.log

{
  echo "== RABAD full start $(date -Is) =="
  ...
  mkdir -p "$PRISMATIC_DATA_ROOT" "$HF_HOME" runs/rabad_lt_main
  : > .hf_token

  echo "== gpu snapshot =="
  nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits
  echo "== command =="
  echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES torchrun --nproc-per-node 2 ... rabad_relation_anchor_w3_tail9_seed7_b20"

  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port 29575 vla_scripts/train.py \
    ...
    --run_root_dir runs/rabad_lt_main \
    --run_id rabad_relation_anchor_w3_tail9_seed7_b20 \
    --save_interval 5000 \
    --tcad_lambda 0.1 \
    --tcad_ratio 0.5 \
    --tcad_margin 0.2 \
  ...
  echo "== RABAD full done $(date -Is) =="
  find runs/rabad_lt_main/rabad_relation_anchor_w3_tail9_seed7_b20 -maxdepth 4 -type f -printf "%p %s\n" | sort
} >> "$LOG" 2>&1
