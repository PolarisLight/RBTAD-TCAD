...
LOG=/mnt/data/cyh/rbtad_smoke_23.log

{
  echo "== RBTAD smoke start $(date -Is) =="
  ...
  mkdir -p "$PRISMATIC_DATA_ROOT" "$HF_HOME" runs/rbtad_lt_smoke
  : > .hf_token

  nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits

  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port 29563 vla_scripts/train.py \
    ...
    --run_root_dir runs/rbtad_lt_smoke \
    --run_id rbtad_w3_tail9_confmedian_s5_seed7 \
    --save_interval 5 \
    --tcad_lambda 0.1 \
    --tcad_ratio 0.5 \
    --tcad_margin 0.2 \
  ...
  echo "== RBTAD smoke done $(date -Is) =="
  tail -20 runs/rbtad_lt_smoke/rbtad_w3_tail9_confmedian_s5_seed7/tcad-debug.csv || true
} >> "$LOG" 2>&1
