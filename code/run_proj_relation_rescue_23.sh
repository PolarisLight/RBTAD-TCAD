...
RUN_ROOT=runs/proj_relation_rescue/${RUN_ID}
LOG=/mnt/data/cyh/${RUN_ID}.log
DO_EVAL=${DO_EVAL:-0}

{
  echo "== Projector-only relation rescue start $(date -Is) =="
  ...
  mkdir -p "$PRISMATIC_DATA_ROOT" "$HF_HOME" runs/proj_relation_rescue
  : > .hf_token
  ...
  set +e
  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port 29593 vla_scripts/train.py \
    ...
    --run_root_dir runs/proj_relation_rescue \
    --run_id "${RUN_ID}" \
    --save_interval 1000 \
    --tcad_lambda 0.05 \
    --tcad_ratio 0.7 \
  ...
  echo "== Projector-only relation rescue train done $(date -Is) =="
  ...
  save_root="results/proj_relation_rescue/${RUN_ID}/${eval_step}/proj_relation_30trials_egl_${stamp}"
  ...
  echo "== Projector-only relation rescue eval done $(date -Is) =="
