...
RUN_ID=${RUN_ID:-relation_pair_rescue_t6_l005_s${LIMIT_STEPS}_seed7_b20}
RUN_ROOT=runs/relation_pair_rescue/${RUN_ID}
LOG=/mnt/data/cyh/${RUN_ID}.log
DO_EVAL=${DO_EVAL:-0}

{
  echo "== Relation-pair rescue start $(date -Is) =="
  ...
  mkdir -p "$PRISMATIC_DATA_ROOT" "$HF_HOME" runs/relation_pair_rescue
  : > .hf_token
  ...
  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port 29592 vla_scripts/train.py \
    ...
    --run_root_dir runs/relation_pair_rescue \
    --run_id "${RUN_ID}" \
    --save_interval 1000 \
    --tcad_lambda 0.05 \
    --tcad_ratio 0.7 \
  ...
  echo "== Relation-pair rescue train done $(date -Is) =="
  ...
  save_root="results/relation_pair_rescue/${RUN_ID}/${eval_step}/relation_pair_30trials_egl_${stamp}"
  ...
  echo "== Relation-pair rescue eval done $(date -Is) =="
