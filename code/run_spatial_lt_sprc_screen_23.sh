...
ANCHOR_L2=${ANCHOR_L2:-0.10}
TCAD_LAMBDA=${TCAD_LAMBDA:-0.1}
TCAD_RATIO=${TCAD_RATIO:-0.5}
RARE_WEIGHT=${RARE_WEIGHT:-2.0}
RUN_STAMP=${RUN_STAMP:-$(date +%Y%m%d_%H%M%S)}
RUN_ROOT=runs/spatial_lt_sprc
RESULT_ROOT=results/spatial_lt_sprc
LOG=/mnt/data/cyh/spatial_lt_sprc_screen_${RUN_STAMP}.log
...
run_seed() {
  ...
  local run_id="sprc_${DATA_MIX}_base1000p${CORRECT_STEPS}_seed${seed}_a${ANCHOR_L2//./p}_${RUN_STAMP}"
  local smoke_id="sprc_smoke_${DATA_MIX}_base1000p5_seed${seed}_a${ANCHOR_L2//./p}_${RUN_STAMP}"
  echo "== seed ${seed} baseline=${base_run} base_ckpt=${base_ckpt} =="

  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port $((30000 + seed)) vla_scripts/train.py \
  ...
  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port $((30100 + seed)) vla_scripts/train.py \
    --pretrained_checkpoint "${base_ckpt}" \
    --vla.type "prism-qwen25-dinosiglip-224px+0_5b+mx-libero-core-lt" \
    --vla.data_mix "${DATA_MIX}" \
    --vla.expected_world_size 2 \
  ...
  run_eval "${run_id}" "${CORRECT_STEPS}" "sprc_seed${seed}"
}

{
  echo "== SPRC screen start $(date -Is) stamp=${RUN_STAMP} =="
  ...
  echo "== SPRC screen all done $(date -Is) =="
} >> "${LOG}" 2>&1
