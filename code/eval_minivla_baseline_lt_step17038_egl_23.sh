...
LOG=/mnt/data/cyh/vla_eval_baseline_lt_step17038_egl_23.log
RUN_NAME=miniVLA_libero_core_lt/prism-qwen25-dinosiglip-224px+0_5b+mx-libero-core-lt+n0+b10+x7
RESULTS_NAME=baseline_lt_step17038_30trials_egl_$(date +"%Y%m%d_%H%M%S")

{
  echo "== baseline lt eval egl start $(date -Is) =="
  ...
  echo "== command =="
  echo "python vla_scripts/parallel_libero_evaluator_egl.py --num-trails-per-task 30 --num-gpus 2 --num-processes 10 --task-suite-name libero_core --pretrained-checkpoint runs/$RUN_NAME --unnorm_key libero_core_lt --save-root results/$RUN_NAME/17038/$RESULTS_NAME --steps 17038 --instruction-formatting False"
  ...
  echo "== baseline lt eval egl done $(date -Is) =="
} >> "$LOG" 2>&1
