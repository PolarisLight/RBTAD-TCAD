...
RESULT_ROOT=results/spatial_lt_action_gain_fixedinit
LOG=/mnt/data/cyh/spatial_lt_action_gain_fixedinit_${RUN_STAMP}.log
...
run_eval() {
  ...
  local save_root="${RESULT_ROOT}/${label}_seed${seed}_gain${gain//./p}_${NUM_TRIALS}trials_initfixed_${RUN_STAMP}"
  echo "== eval start $(date -Is) seed=${seed} label=${label} gain=${gain} init_ids=${EVAL_INIT_IDS} save_root=${save_root} =="
...
{
  echo "== Tail action-gain fixed-init diagnostic start $(date -Is) stamp=${RUN_STAMP} gain=${GAIN} tail_ids=${TAIL_TASK_IDS} init_ids=${EVAL_INIT_IDS} =="
...
PY
  ...
  echo "== Tail action-gain fixed-init diagnostic all done $(date -Is) =="
} >> "${LOG}" 2>&1