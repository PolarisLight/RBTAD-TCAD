...
LOG=/mnt/data/cyh/rctad_full_23.log

{
  echo "== RCTAD full start $(date -Is) =="
  ...
  mkdir -p "$PRISMATIC_DATA_ROOT" "$HF_HOME" runs/rctad_lt_main
  : > .hf_token

  echo "== gpu snapshot =="
  nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits
  echo "== command =="
  echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES torchrun --nproc-per-node 2 ... rctad_tail9_confmedian_ratio05_seed7_b20"

  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port 29562 vla_scripts/train.py \
    ...
    --run_root_dir runs/rctad_lt_main \
  ...
  echo "== RCTAD full done $(date -Is) =="
  find runs/rctad_lt_main/rctad_tail9_confmedian_ratio05_seed7_b20 -maxdepth 4 -type f -printf "%p %s\n" | sort
} >> "$LOG" 2>&1
