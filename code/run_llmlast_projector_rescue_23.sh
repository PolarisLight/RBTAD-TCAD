...
RUN_ID=${RUN_ID:-llmlast_projector_rescue_t6_l003_s${LIMIT_STEPS}_seed7_b20}
RUN_ROOT=runs/llmlast_projector_rescue/${RUN_ID}
LOG=/mnt/data/cyh/${RUN_ID}.log
DO_EVAL=${DO_EVAL:-0}

{
  echo "== LLM-last + projector relation rescue start $(date -Is) =="
  ...
  mkdir -p "$PRISMATIC_DATA_ROOT" "$HF_HOME" runs/llmlast_projector_rescue
  : > .hf_token
  ...
  set +e
  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port 29594 vla_scripts/train.py \
    ...
    --run_root_dir runs/llmlast_projector_rescue \
    --run_id "${RUN_ID}" \
    --save_interval 1000 \
    --tcad_lambda 0.03 \
    --tcad_ratio 0.7 \
    ...
    --anchor_l2_filter "" \
    --extra_trainable_filter "projector" \
    --train_limit_steps "${LIMIT_STEPS}"
  train_status=$?
  set -e

  echo "== LLM-last + projector relation rescue train done $(date -Is) =="
  ...
  save_root="results/llmlast_projector_rescue/${RUN_ID}/${eval_step}/llmlast_projector_30trials_egl_${stamp}"
  ...
  echo "== LLM-last + projector relation rescue eval done $(date -Is) =="
