...
RUN_ROOT=${ROOT}/runs/selective_interp_projector/${RUN_ID}
...
{
  echo "== Selective projector interp start $(date -Is) alpha=${ALPHA} include=${INCLUDE_PREFIXES} =="
  ...
  if [[ ! -f "${OUT}" ]]; then
    args=(
      /mnt/data/cyh/make_interp_ckpt.py
      --base "${BASE}"
      --delta "${DELTA}"
      --out "${OUT}"
      --alpha "${ALPHA}"
    ...
    IFS=',' read -ra prefixes <<< "${INCLUDE_PREFIXES}"
    for prefix in "${prefixes[@]}"; do
      args+=(--include-prefix "${prefix}")
    done
    /mnt/data/cyh/envs/vla-long-tail/bin/python "${args[@]}"
  else
  ...
  save_root="results/selective_interp_projector/${RUN_ID}/0/${TAG}_30trials_egl_${stamp}"
  ...
  python vla_scripts/parallel_libero_evaluator_egl.py \
    ...
    --pretrained-checkpoint "runs/selective_interp_projector/${RUN_ID}" \
    --unnorm_key libero_core_lt \
    --save-root "${save_root}" \
    --steps 0 \
    --instruction-formatting False

  echo "== Selective projector interp eval done $(date -Is) =="
