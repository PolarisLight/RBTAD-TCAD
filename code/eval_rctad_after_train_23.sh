...
LOG=/mnt/data/cyh/rctad_eval_after_train_23.log

{
  echo "== RCTAD eval watcher start $(date -Is) =="
  while pgrep -af "train_rctad_full_23|vla_scripts/train.py" >/dev/null; do
    echo "waiting for RCTAD training $(date -Is)"
    sleep 1800
  done
  ...
  save_root="results/rctad_lt_main/$RUN_ID/$latest_ckpt/rctad_step${latest_ckpt}_30trials_egl_${stamp}"
