...
LOG=/mnt/data/cyh/spatial_lt_rlct_screen_${RUN_STAMP}.log
...
run_seed() {
  ...
  local run_id="rlct_${DATA_MIX}_base1000p${CORRECT_STEPS}_seed${seed}_a${ANCHOR_L2//./p}_${RUN_STAMP}"
  local smoke_id="rlct_smoke_${DATA_MIX}_base1000p5_seed${seed}_a${ANCHOR_L2//./p}_${RUN_STAMP}"
  ...
  run_eval "${run_id}" "${CORRECT_STEPS}" "rlct_seed${seed}"
}

{
  echo "== RLCT screen start $(date -Is) stamp=${RUN_STAMP} =="
  ...
  echo "== RLCT screen all done $(date -Is) =="
} >> "${LOG}" 2>&1
