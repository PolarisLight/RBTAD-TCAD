...
RUN_ROOT=runs/rabad_lt_main/$RUN_ID
LOG=/mnt/data/cyh/rabad_eval_after_train_23.log

{
  echo "== RABAD eval watcher start $(date -Is) =="
  while pgrep -af "train_rabad_full_23|vla_scripts/train.py" >/dev/null; do
    echo "waiting for RABAD training $(date -Is)"
    sleep 1800
  done
  ...
  save_root="results/rabad_lt_main/$RUN_ID/$latest_ckpt/rabad_step${latest_ckpt}_30trials_egl_${stamp}"
