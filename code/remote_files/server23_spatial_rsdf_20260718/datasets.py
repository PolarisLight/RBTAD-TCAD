...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        gripper_value = float(np.asarray(action)[-1])
        ...
        target_task_phase = os.environ.get("TARGET_TASK_PHASE", "all").strip().lower()
        if target_task_phase in {"pre_grasp", "open_gripper", "gripper_nonpositive"}:
            target_phase_allowed = gripper_value <= 0
        elif target_task_phase in {"post_grasp", "closed_gripper", "gripper_positive"}:
            target_phase_allowed = gripper_value > 0
        else:
            target_phase_allowed = True
        if (
            target_task_instructions
            and lang.lower() in target_task_instructions
            and target_task_weight != 1.0
            and target_phase_allowed
        ):
        ...
        if tcad_ratio > 0 and random.random() < tcad_ratio:
            ...
            phase_mode = os.environ.get("TCAD_PHASE", "gripper_positive").strip().lower()
            if phase_mode in {"pre_grasp", "open_gripper", "gripper_nonpositive"}:
                phase_allowed = gripper_value <= 0
            elif phase_mode in {"all", "none"}:
                phase_allowed = True
            else:
                phase_allowed = gripper_value > 0
            tail_max_count = int(os.environ.get("TCAD_TAIL_MAX_COUNT", "0"))
            tail_allowed = tail_max_count <= 0 or task_count <= tail_max_count
            if wrong_lang is not None and phase_allowed and tail_allowed:
                neg_conversation = [
