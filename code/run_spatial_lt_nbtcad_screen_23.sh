...
RARE_WEIGHT=${RARE_WEIGHT:-1.0}
RUN_STAMP=${RUN_STAMP:-$(date +%Y%m%d_%H%M%S)}
RUN_ROOT=runs/spatial_lt_nbtcad
RESULT_ROOT=results/spatial_lt_nbtcad
LOG=/mnt/data/cyh/spatial_lt_nbtcad_screen_${RUN_STAMP}.log

wait_for_gpus() {
  echo "== waiting for GPUs 2/3 $(date -Is) =="
  while true; do
    local used2 used3 util2 util3 procs
    ...
    procs=$(nvidia-smi --query-compute-apps=gpu_uuid,pid,used_memory --format=csv,noheader,nounits 2>/dev/null || true)
    echo "$(date -Is) gpu2=${used2}MiB/${util2}% gpu3=${used3}MiB/${util3}%"
    if [[ -n "${procs}" ]]; then echo "${procs}"; fi
...
run_seed() {
  ...
  local run_id="nbtcad_${DATA_MIX}_base1000p${CORRECT_STEPS}_seed${seed}_a${ANCHOR_L2//./p}_${RUN_STAMP}"
  local smoke_id="nbtcad_smoke_${DATA_MIX}_base1000p5_seed${seed}_a${ANCHOR_L2//./p}_${RUN_STAMP}"
  echo "== seed ${seed} baseline=${base_run} base_ckpt=${base_ckpt} =="

  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port $((30400 + seed)) vla_scripts/train.py \
  ...
  torchrun --nnodes 1 --nproc-per-node 2 --master_addr 127.0.0.1 --master_port $((30500 + seed)) vla_scripts/train.py \
    --pretrained_checkpoint "${base_ckpt}" \
    --vla.type "prism-qwen25-dinosiglip-224px+0_5b+mx-libero-core-lt" \
    --vla.data_mix "${DATA_MIX}" \
    --vla.expected_world_size 2 \
  ...
  run_eval "${run_id}" "${CORRECT_STEPS}" "nbtcad_seed${seed}"
}

{
  echo "== NB-TCAD screen start $(date -Is) stamp=${RUN_STAMP} =="
  ...
  python /mnt/data/cyh/patch_negative_branch_tcad.py
  python - <<'PY'
from pathlib import Path
for path in [Path('prismatic/training/strategies/base_strategy.py'), Path('vla_scripts/train.py')]:
...
PY
  ...
  echo "== NB-TCAD screen all done $(date -Is) =="
} >> "${LOG}" 2>&1
